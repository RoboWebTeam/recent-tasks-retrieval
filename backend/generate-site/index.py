...
# Общий «контракт разметки» (Фаза 0 «Экспорта в код»): парсеры data-rw/служебных маркеров вынесены
# в backend/_shared/rw_markup.py — единый источник и для генератора, и для экспортёра кода.
# server.py кладёт в sys.path папку функции; добавляем ещё и backend/, чтобы найти пакет _shared.
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from _shared.rw_markup import (  # noqa: E402
...
- Сам добавляй очевидно полезные для этой ниши элементы, даже если их не просили напрямую (кнопка записи/заказа, блок доверия, контакты). Сомневаешься, делать блок или нет — делай: недоделанный проект это брак, избыточный — нет.
...
6a. ИЗОБРАЖЕНИЯ — используй КАЧЕСТВЕННЫЕ фото, СООТВЕТСТВУЮЩИЕ тематике сайта (это сильно поднимает «дорогой» вид) — но ТОЛЬКО из гарантированно работающих источников. НИКОГДА не выдумывай URL картинок и не бери source.unsplash.com (ЗАКРЫТ, не работает), unsplash-ссылки с ID, случайные/вымышленные пути — они не грузятся и показывают сломанный значок.
   ...
   Картинки: на лендинг 8-16 осмысленных фото (галерея 4-8, 1-2 в hero/секциях), а в каталоге товаров — по одной на КАЖДУЮ карточку (12-24 нормально). Верхнего предела нет, но каждая картинка должна быть по смыслу.
...
8. ПОЛНОТА И КРАСОТА ВМЕСТЕ: по умолчанию собирай ПОЛНЫЙ премиум-лендинг из 9-11 секций (см. блок ПРЕМИУМ-ДИЗАЙН, раздел «Богатый контент»). Заготовка из hero + 3 карточек — брак. Бюджет вывода БОЛЬШОЙ — экономить на объёме НЕЛЬЗЯ. Пиши читаемый CSS, переиспользуй классы и CSS-переменные ради качества кода, а не ради экономии места. ВЫБРАСЫВАТЬ СЕКЦИИ ЗАПРЕЩЕНО: если кажется, что объёма мало — это ошибка оценки, продолжай верстать и ОБЯЗАТЕЛЬНО закончи документ на </body></html>.
9. ЦЕЛОСТНОСТЬ И ПОЛНОТА: документ ОБЯЗАТЕЛЬНО заканчивается на </body></html> и НИКОГДА не обрывается на середине тега/CSS. Бюджет токенов большой — места хватает на ПОЛНЫЙ сайт, поэтому НЕ экономь на секциях и НЕ останавливайся раньше времени. КРИТИЧНО: доведи до конца ВСЕ секции из своего плана RW:plan и весь обязательный скелет из блока ПРЕМИУМ-ДИЗАЙН (раздел 4). Если чувствуешь, что объёма мало — это ошибка оценки: места хватает, продолжай. Секции НЕ выбрасывай и не сокращай. Закрыть </html> без меню/услуг с ценами, без тарифов, без отзывов, без FAQ (<details>) или без футера — это БРАК, так делать НЕЛЬЗЯ.
...
# _valid_db_identifier, extract_schema_block → _shared.rw_markup (импорт вверху файла)
...
# strip_injected_runtime → _shared.rw_markup (импорт вверху файла)
...
# derive_schema_from_html, merge_schema → _shared.rw_markup (импорт вверху файла)
...
# extract_fn_blocks → _shared.rw_markup (импорт вверху файла)
...
5. МНОГОСТРАНИЧНОЕ ПРИЛОЖЕНИЕ (SPA: страницы, навигация, корзина). Для приложений (интернет-магазин, каталог+корзина+оформление, многоэкранный сервис) делай НЕСКОЛЬКО экранов в ОДНОМ файле — платформа сама делает роутинг между data-rw-page, отправку форм, чтение каталогов, аккаунты, корзину и вызов серверных функций — их дублировать НЕ надо. Всю ОСТАЛЬНУЮ интерактивность пиши сам обычным JS и не экономь: фильтры, поиск, сортировку, табы, аккордеоны, модалки, перетаскивание карточек канбана, пересчёты в формах, графики инлайн-SVG, всплывающие уведомления, скелетоны загрузки. 200-400 строк своего JS для приложения — норма. ВАЖНО: имя window.rw занято платформой — никогда не объявляй и не перезаписывай его.
...
Если по смыслу запроса нужны вход, личные данные, заказы, брони, задачи, статусы или история — это ПРИЛОЖЕНИЕ: делай экраны data-rw-page и аккаунты, даже если слов «магазин» или «кабинет» в запросе не было. И наоборот: простому лендингу страницы и корзина не нужны — там не усложняй."""
...
# Тип продукта, выбранный в редакторе. В отличие от STYLE_PRESETS (это ЭСТЕТИКА) здесь задаётся
# КЛАСС проекта. Раньше класс угадывался только по словам в запросе: «магазин обуви» давал
# wants_app, но не попадал в крупную задачу (одно ключевое слово, короткий текст) — и многоэкранное
# приложение генерировалось с бюджетом лендинга, обрываясь на середине. Явный выбор в интерфейсе
# снимает угадывание: он включает нужные возможности детерминированно.
PRODUCT_TYPES = {
    'landing': {
        'app': False, 'accounts': False, 'large': True,
        'hint': "ТИП ПРОЕКТА — ЛЕНДИНГ: одна длинная продающая страница из 9-11 проработанных секций "
                "с якорной навигацией и формой заявки. Страницы data-rw-page и корзина НЕ нужны.",
    },
    'store': {
        'app': True, 'accounts': True, 'large': True,
        'hint': "ТИП ПРОЕКТА — ИНТЕРНЕТ-МАГАЗИН: обязательно многоэкранное приложение (раздел 5) — "
                "витрина, каталог с фильтрами и поиском, карточка товара, корзина с подсчётом суммы, "
                "оформление заказа и личный кабинет с историей заказов (раздел 4). Товары — в таблице "
                "products, заказы — в owner_scoped таблице orders.",
    },
    'portal': {
        'app': True, 'accounts': True, 'large': True,
        'hint': "ТИП ПРОЕКТА — ПОРТАЛ/ПЛАТФОРМА: многоэкранное приложение (раздел 5) с каталогом записей "
                "(объявления, курсы, врачи, объекты — по нише), развёрнутыми фильтрами и поиском, "
                "детальной карточкой записи, формой подачи/бронирования и личным кабинетом (раздел 4), "
                "где видно свои записи и заявки.",
    },
    'saas': {
        'app': True, 'accounts': True, 'large': True,
        'hint': "ТИП ПРОЕКТА — РАБОЧАЯ ПАНЕЛЬ (SaaS/CRM/дашборд): многоэкранное приложение (раздел 5) с "
                "боковым меню, экраном обзора с KPI-плитками и графиком на инлайн-SVG, экраном-таблицей "
                "записей с фильтрами и созданием/редактированием, экраном настроек и входом с личным "
                "кабинетом (раздел 4). Данные — в owner_scoped таблицах, чтобы каждый видел только своё.",
    },
}
...
def _handler_impl(event: dict, context) -> dict:
    ...
    wants_app = any(k in _lu_intent for k in (
        'магазин', 'корзин', 'маркетплейс', 'checkout', 'приложени', 'многостранич',
        'оформить заказ', 'оформление заказ',
        # Порталы и рабочие панели — тоже многоэкранные продукты, а не лендинги. Без этих слов
        # запрос «сделай CRM для отдела продаж» собирался как одностраничник с описанием CRM.
        'портал', 'платформ', 'crm', 'дашборд', 'dashboard', 'панель управлени', 'админк',
        'канбан', 'бронирован', 'запись на приём', 'база данных', 'таблица заявок',
    ))
    ...
    # Тип продукта выбран в редакторе — это надёжнее угадывания по словам, поэтому он ДОБАВЛЯЕТ
    # возможности к распознанным (но не отменяет их: если человек выбрал «Лендинг», а в тексте
    # прямо просит корзину, корзина всё равно нужна).
    product_choice = str(body.get('product_type', '') or '').strip().lower()
    product = PRODUCT_TYPES.get(product_choice) if not is_edit else None
    if product:
        wants_app = wants_app or product['app']
        wants_accounts = wants_accounts or product['accounts']

    is_large_task = detect_large_task(last_user_text, is_edit)
    # Многоэкранное приложение или аккаунты — это всегда крупная задача, даже если запрос короткий
    # («сделай магазин обуви»). Иначе такому проекту доставался бюджет токенов лендинга, и он
    # обрывался на середине: экраны без разметки, битая корзина, белый экран.
    if not is_edit and (wants_app or wants_accounts or (product and product['large'])):
        is_large_task = True
    ...
    # Порог «усыхания» проекта при правке: если ответ короче исходного больше чем на эту долю,
    # значит модель потеряла содержимое — правку не сохраняем. Прежние 0.7 были рассчитаны на
    # маленькие сайты: у проекта на 120КБ они молча пропускали потерю трети содержимого.
    SHRINK_GUARD = 0.92
    # Если пользователь САМ просит что-то убрать, проект законно становится короче — там
    # держим только грубую страховку от полного стирания.
    SHRINK_GUARD_REMOVAL = 0.5
    if current_html:
...
5. КРИТИЧЕСКИ ВАЖНО: документ ОБЯЗАТЕЛЬНО должен быть завершён и заканчиваться на </body></html>. НИКОГДА не обрывай код на середине тега, стиля или блока — иначе сайт сломается и блоки пропадут. Если правка большая — всё равно уложись и заверши документ целиком."""
    ...
    #  - 'sonnet' → режим «Стандарт» (быстрее и дешевле, отличный баланс);
    #  - 'opus'   → режим «Максимум» (мощнее для сложных/крупных проектов).
    # Обе идут напрямую через Anthropic API (без посредников вроде OpenRouter).
    # Неизвестное/пустое значение → безопасный дефолт «Стандарт».
    ANTHROPIC_MODELS = {
    ...
    # Модели этого поколения физически НЕ укладываются в 30-секундный лимит функции: они
    # долго «рассуждают» и упираются в жёсткий таймаут платформы (504) раньше, чем мы успеваем
    ...
    if function_timeout <= 35:
        ...
        return err('Сборка не успевает завершиться за текущий таймаут функции. Поднимите таймаут generate-site до 90 секунд в настройках (Ядро → Функции).', 400)
    ...
    # ── РЕЖИМ РАССУЖДЕНИЯ ──────────────────────────────────────────────────────────────
    # Модели этого поколения умеют «думать» перед ответом, и именно это даёт им силу на
    # сложных проектах. Управление — ТОЛЬКО через thinking={'type':'adaptive'} и
    # output_config.effort (проверено на живом API: старый thinking={'type':'enabled'} с
    # budget_tokens эти модели отвергают с 400).
    #
    # ГЛАВНЫЕ ГРАБЛИ: токены рассуждения списываются из ТОГО ЖЕ max_tokens, что и код сайта.
    # Если включить рассуждение, не подняв лимит, документ оборвётся на середине. Поэтому под
    # рассуждение резервируется отдельная добавка к бюджету, а не отбирается место у HTML.
    #
    # Уровень усилия выбираем по типу задачи: правка — это в основном воспроизведение уже
    # готового файла, глубокое рассуждение там не окупается; многоэкранное приложение —
    # наоборот, самая сложная задача редактора.
    THINKING_MODELS = ('claude-opus-5', 'claude-sonnet-5')
    _think_on = ANTHROPIC_MODEL.startswith(THINKING_MODELS)
    if current_html:
        ai_effort = 'medium'
    elif wants_app or wants_accounts:
        ai_effort = 'xhigh'
    else:
        ai_effort = 'high'
    THINK_RESERVE = {'low': 3000, 'medium': 8000, 'high': 16000, 'xhigh': 32000}[ai_effort] if _think_on else 0

    # ── БЮДЖЕТ ВЫВОДА ─────────────────────────────────────────────────────────────────
    # Потолок моделей — 128000 выходных токенов. Настоящее ограничение не в нём, а во ВРЕМЕНИ:
    # замеры на живом API дали ~90 токенов/с в режиме «Максимум» и ~126 в «Стандарте», то есть
    # полные 128000 токенов — это 17-24 минуты ожидания, чего живой клиент не выдержит.
    #
    # Поэтому бюджет считается не магическим числом, а от того, сколько времени не жалко ждать:
    # бюджет = целевое время × реальная скорость модели. Быстрая модель за то же время успевает
    # больше — и получает больший бюджет автоматически, без отдельных констант.
    #
    # В СТРИМ-режиме таймаут сокета срабатывает НА КАЖДЫЙ чанк, а не на всю генерацию, и nginx
    # рвёт связь только при 180с ТИШИНЫ (keepalive идёт каждые 10с) — поэтому длинная сборка
    # спокойно доходит до конца, а пользователь видит прогресс живьём.
    MODEL_TPS = 126 if _model_key == 'sonnet' else 90  # замерено на живом API
    if function_timeout <= 35:
        # Таймаут функции не поднят — сюда мы вообще не доходим (выше стоит отказ), но оставляем
        # безопасный минимум на случай ручного запуска.
        max_tokens = 6500 if is_large_task else 5000
    elif not stream_mode:
        # Буферный режим (откат, когда стрим не стартовал) ограничен таймаутом функции целиком.
        max_tokens = min(int(ai_timeout * MODEL_TPS * 0.8), 22000 if is_large_task else 16000)
    else:
        # Целевое ВРЕМЯ живой сборки — в него входит и рассуждение, и код (они идут одним потоком
        # с одинаковой скоростью). Поэтому худшее время ожидания равно целевому, а не больше него.
        # Лестница: обычный проект ~5 мин, крупный ~7, многоэкранное приложение ~10 — это самая
        # тяжёлая задача редактора, и там ожидание оправдано.
        target_sec = 600 if (wants_app or wants_accounts) else (420 if is_large_task else 300)
        max_tokens = min(int(target_sec * MODEL_TPS), 128000)
    ...
    #  - но НЕ больше, чем модель реально успеет сгенерировать за бюджет времени.
    if current_html:
        # ЧЕСТНЫЙ ГАРД. Раньше сайт длиннее MAX_HTML_CHARS молча обрезался перед отправкой, и
        # модель возвращала обрезок как «весь сайт» — тихая потеря хвоста. Лучше внятный отказ.
        if len(current_html) > MAX_HTML_CHARS:
            refund_quota(user_id, schema, request_cost)
            return err('Проект уже очень большой — переписать его целиком за одну правку не получится, часть блоков потерялась бы. Проект оставлен без изменений. Попросите точечное изменение (например: «измени цвет кнопок» или «поправь текст в блоке услуг») — такие правки применяются надёжно.', 413)
        # Сколько токенов нужно, чтобы вернуть весь сайт. Премиум-проекты плотные (инлайн-SVG,
        # CSS, кириллица) — делитель 2.0 занижал потребность на четверть и правки обрывались;
        # по замерам этого поколения ближе к 1.6 символа на токен.
        html_tokens = int(len(html_for_prompt) / 1.6)
        # Надбавка на сами изменения. Плоские +8000 не покрывали запрос вроде «добавь три секции»
        # на премиум-скелете — это десятки тысяч новых токенов, поэтому надбавка процентная.
        needed_text = html_tokens + max(8000, int(html_tokens * 0.25))
        needed = needed_text + THINK_RESERVE
        # В стрим-режиме потолок скорости не жмём (таймаут на чанк); в буферном — жмём по
        # реально замеренной скорости модели с запасом 20%.
        speed_ceiling = 10_000_000 if stream_mode else int(ai_timeout * MODEL_TPS * 0.8)
        hard_ceiling = 12000 if function_timeout <= 35 else (96000 if stream_mode else 32000)
        ceiling = min(hard_ceiling, speed_ceiling, 128000)
        # Порог отказа считаем по объёму САМОГО ТЕКСТА, без резерва на рассуждение — иначе
        # включение рассуждения само по себе начало бы отклонять правки, которые раньше проходили.
        if needed_text > ceiling * 1.4:
            refund_quota(user_id, schema, request_cost)
            return err('Проект уже большой — переписать его целиком за одну правку не получится, часть блоков могла бы потеряться. Проект оставлен без изменений. Попросите точечное изменение (например: «измени цвет кнопок» или «поправь текст в блоке услуг») — такие правки применяются надёжно.', 413)
        max_tokens = min(needed, ceiling)
    ...
    # Тип продукта из редактора идёт ПОСЛЕ скелета лендинга: он описывает класс проекта целиком
    # и должен доминировать над общим хинтом «расширенный лендинг».
    if product:
        effective_system_prompt += "\n\n" + product['hint']
    ...
    # БЮДЖЕТ ОТВЕТА — модели надо знать, сколько у неё места. Без этого она планирует объём вслепую:
    # на живом прогоне «магазин электроники» она начала строить приложение на 9 экранов и оборвалась
    # на середине, потому что не соотнесла замысел с лимитом. Незавершённый документ хуже, чем
    # меньший, но целый, поэтому просим её сначала урезать замысел, а не бросать вёрстку на полуслове.
    _text_budget = max(4000, max_tokens - THINK_RESERVE)
    effective_system_prompt += (
        f"\n\nБЮДЖЕТ ЭТОГО ОТВЕТА — примерно {_text_budget // 1000} тысяч токенов на весь код. "
        "Это МНОГО, на полноценный проект хватает с запасом — не экономь на качестве. Но перед началом "
        "соотнеси свой замысел с этим бюджетом и спланируй так, чтобы ГАРАНТИРОВАННО закрыть "
        "</body></html> внутри него. Если замысел явно не помещается — сократи ЗАМЫСЕЛ заранее "
        "(меньше экранов или карточек), а не бросай вёрстку на середине. Оборванный документ — брак: "
        "шесть доведённых до конца экранов лучше девяти незаконченных."
    )
    ...
    def call_anthropic(model_id: str, override_messages=None, override_max_tokens=None, override_timeout=None):
        ...
        # Рассуждение перед ответом — то, что даёт моделям этого поколения силу на сложных
        # проектах. ВАЖНО: temperature/top_p/top_k при включённом рассуждении API отвергает,
        # поэтому их здесь нет и добавлять их нельзя.
        if model_id.startswith(THINKING_MODELS):
            payload_obj['thinking'] = {'type': 'adaptive'}
            payload_obj['output_config'] = {'effort': ai_effort}
    ...
    if stream_mode:
        ...
        def _stream_gen():
            ...
            refunded = False
            try:
                # Клиенту отдаём РЕЖИМ, а не идентификатор движка: название движка не должно
                # утекать даже в тело SSE (оно видно в инструментах разработчика браузера).
                # Реальный ID пишется в лог и в метрики — это сервер и админка, там можно.
                yield _sse('start', {'mode': 'enhanced' if _model_key == 'opus' else 'standard',
                                     'is_edit': is_edit, 'cost': request_cost})
                anthropic_messages = [
                ...
                # с большой задержкой (модель сначала долго «думает»), поэтому запас большой —
                # 240с на один чанк. Иначе фаза рассуждения рвёт стрим (read timed out).
                import time as _time
                _last_beat = _time.time()
                # Ограничитель ОБЩЕГО времени стрима. Раньше его не было вовсе: зависшая генерация
                # держала поток вечно, а при 4 воркерах × 16 потоков несколько таких запросов
                # выводят сервер из строя целиком, включая вход в кабинет.
                _stream_started = _time.monotonic()
                STREAM_WALL_SEC = int(os.environ.get('STREAM_WALL_SEC', '900'))
                with urllib.request.urlopen(req, timeout=max(240, ai_timeout)) as response:
                    for raw_line in response:
                        if _time.monotonic() - _stream_started > STREAM_WALL_SEC:
                            raise TimeoutError(f'stream wall {STREAM_WALL_SEC}s exceeded')
                        ...
                        if etype == 'message_start':
                            ...
                            # Модель начала думать. Показываем фазу человеку, чтобы он не смотрел
                            # в тишину: рассуждение может занять десятки секунд до первого кода.
                            # Отдаём СВОЙ ярлык, а не текст модели — он мог бы назвать движок.
                            if ((evt.get('content_block') or {}).get('type')) == 'thinking':
                                _last_beat = _time.time()
                                yield _sse('phase', {'stage': 'thinking'})
                            ...
                            elif _dt in ('thinking_delta', 'signature_delta'):
                                # КРИТИЧНО: рассуждение НИКОГДА не попадает в full_parts — иначе оно
                                # окажется внутри HTML-документа, тот не пройдёт проверку </html>
                                # и сборка будет отклонена. Только сбрасываем счётчик keepalive.
                                _last_beat = _time.time()
                ...
                if is_edit and was_truncated:
                    ...
                    return
                if is_edit and not was_truncated and len(html_out) < len(current_html) * (SHRINK_GUARD_REMOVAL if is_removal_request(last_user_content) else SHRINK_GUARD):
                    refund_quota(user_id, schema, request_cost); refunded = True
    ...
    if is_edit and not was_truncated and len(html) < len(current_html) * (SHRINK_GUARD_REMOVAL if is_removal_request(last_user_content) else SHRINK_GUARD):
        refund_quota(user_id, schema, request_cost)
    ...
    # ВТОРОЙ («арт-директорский») ПРОХОД УДАЛЁН. Причины, каждая самодостаточна:
    #  1) Он почти не выполнялся: условие требовало НЕ-стрим, а редактор всегда идёт стримом —
    #     буферный запрос остался только откатом на случай, если стрим не стартовал.
    #  2) Он мог затереть целый проект оборванным ответом: repair_truncated_html() ВСЕГДА дописывает
    #     </body></html>, поэтому проверка «заканчивается на </html>» пропускала обрезанный результат,
    #     а проверок длины и числа секций там не было вовсе.
    #  3) Модели этого поколения проверяют себя сами в рассуждении — внешний проход «перепроверь»
    #     даёт разрастание вывода и удвоение стоимости без прироста качества.
    #  4) Он ломал кэш промпта: подменял системный промпт и уводил build_system_blocks в откат.
    # Требования к качеству, которые он нёс, перенесены в основной системный промпт.
