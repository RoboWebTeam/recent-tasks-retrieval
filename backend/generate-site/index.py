...
6a. ИЗОБРАЖЕНИЯ — КРИТИЧЕСКИ ВАЖНО (частая причина «битых» картинок): НИКОГДА не выдумывай ссылки на картинки и не бери случайные URL (example.com, unsplash-ссылки с ID, source.unsplash.com — этот сервис ЗАКРЫТ и НЕ работает, вымышленные пути) — они не загружаются и превью показывает сломанный значок. По умолчанию для hero, фоновых блоков и любых декоративных секций используй CSS linear-gradient / radial-gradient и CSS-фигуры вместо <img> — это всегда красиво и НИКОГДА не ломается. Если картинка реально нужна, бери ТОЛЬКО гарантированно работающие источники:
   - Реалистичное фото: https://picsum.photos/seed/КЛЮЧЕВОЕ-СЛОВО/1600/900 (seed фиксирует конкретное фото — например /seed/coffee/1600/900; всегда отдаёт реальное изображение);
   - Заглушки/аватары/логотипы: https://placehold.co/400x400?text=Текст или эмодзи/иконки/CSS-фигуры.
   У каждого <img> ОБЯЗАТЕЛЬНО указывай alt, width/height и style="object-fit:cover" — чтобы вёрстка не «прыгала», даже если изображение долго грузится.
...
9b. ЖИВЫЕ МАРКЕРЫ ПРОГРЕССА (для интерактивного чата — ОБЯЗАТЕЛЬНО): непосредственно ПЕРЕД каждой крупной секцией <body> (hero, преимущества, услуги/меню/каталог, отзывы, форма/контакты, footer и т.п.) вставляй служебный HTML-комментарий СТРОГО в формате <!--RW:step:краткое живое описание что делаешь--> — например <!--RW:step:Собираю hero с крупным оффером и кнопкой записи--> или <!--RW:step:Верстаю меню с ценами и фото-->. Пиши от первого лица, живо и по делу, 3-7 слов, каждый маркер РАЗНЫЙ. Всего 4-8 маркеров (по числу реальных секций). Это невидимые комментарии — они НЕ влияют на вид сайта, но в реальном времени показывают пользователю ход твоей работы. При ПРАВКЕ ставь маркеры о том, что именно меняешь (например <!--RW:step:Меняю цвет кнопок на синий-->). Не злоупотребляй — только перед настоящими секциями/изменениями.
...
def strip_progress_markers(html: str) -> str:
    """Убирает служебные маркеры живого прогресса <!--RW:step:...--> из ФИНАЛЬНОГО HTML.
    В SSE-потоке они нужны фронтенду, чтобы показывать шаги работы в реальном времени, но в
    сохранённом/готовом/экспортируемом документе их быть не должно."""
    import re
    return re.sub(r'<!--\s*RW:step:.*?-->\s*', '', html, flags=re.DOTALL)
...
3. КРИТИЧЕСКИ ВАЖНО: документ ОБЯЗАТЕЛЬНО должен быть завершён и заканчиваться на </body></html>. НИКОГДА не обрывай код на середине тега, стиля или блока — иначе сайт сломается и блоки пропадут. Если правка большая — всё равно уложись и заверши документ целиком."""
    ...
    if current_html:
        # Сколько токенов реально нужно, чтобы вернуть весь сайт. 1 токен ≈ 3.3 символа для
        # смешанного RU/HTML; берём с небольшим запасом (÷3) + 900 на сами изменения (мелкая
        # правка типа цвета/текста почти не меняет объём — большой запас тут не нужен и только
        # зря сужает потолок, из-за чего простые правки на больших сайтах ложно блокировались).
        needed = len(current_html) // 3 + 900
        ...
        hard_ceiling = 10000 if function_timeout <= 35 else 20000
