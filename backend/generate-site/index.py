...
3. КРИТИЧЕСКИ ВАЖНО: документ ОБЯЗАТЕЛЬНО должен быть завершён и заканчиваться на </body></html>. НИКОГДА не обрывай код на середине тега, стиля или блока — иначе сайт сломается и блоки пропадут. Если правка большая — всё равно уложись и заверши документ целиком."""
    ...
    if current_html:
        # Сколько токенов реально нужно, чтобы вернуть весь сайт + запас на добавление секции.
        # 1 токен ≈ 3.3 символа для смешанного RU/HTML; берём с запасом (÷3) + 2500 на новый блок.
        needed = len(current_html) // 3 + 2500
        # Сколько модель реально успеет сгенерировать за отведённое время (быстрые модели ~170 ток/сек).
        speed_ceiling = int(ai_timeout * 170)
        hard_ceiling = 8000 if function_timeout <= 35 else 16000
        ceiling = min(hard_ceiling, speed_ceiling)
        # Если для целостной правки нужно БОЛЬШЕ, чем модель успеет сгенерировать — правка
        # физически не поместится и сайт оборвётся. НЕ пытаемся (иначе теряются блоки),
        # а честно просим правку попроще, сохранив текущий сайт целым.
        if needed > ceiling:
            refund_quota(user_id, schema, request_cost)
        ...
        # Даём ровно столько, сколько нужно на весь сайт + запас (в пределах потолка).
