...
5. КРИТИЧЕСКИ ВАЖНО: документ ОБЯЗАТЕЛЬНО должен быть завершён и заканчиваться на </body></html>. НИКОГДА не обрывай код на середине тега, стиля или блока — иначе сайт сломается и блоки пропадут. Если правка большая — всё равно уложись и заверши документ целиком."""
    ...
    if current_html:
        # Сколько токенов реально нужно, чтобы вернуть весь сайт. ПРЕМИУМ-сайты плотные (инлайн-SVG,
        # CSS, кириллица) → ~2.0 символа/токен (делитель 2.5 занижал: 41КБ-сайт упирался в max_tokens
        # ~19000 и правка обрывалась на середине → «не поместилась»). + 8000 на сами изменения (правка
        # часто ДОБАВЛЯЕТ целую секцию) и служебные блоки ROBOWEB_META/RW:report в каждом ответе.
        needed = int(len(current_html) / 2.0) + 8000
