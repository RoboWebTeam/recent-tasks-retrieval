...
const SCRIPT_RU = [
  { time: 2,  type: 'user', text: 'Нужен проект для кофейни «Brew & Co» — меню, онлайн-заказ и заявки в базу.' },
  ...
  { time: 43, type: 'done', text: 'Готово: рабочий проект с бэкендом. Код Next.js + Prisma — в вашем GitHub, данные — в вашей PostgreSQL.' },
];
...
export default function DemoModal({ open, onClose, lang }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={lang === 'ru' ? 'Демонстрация сборки проекта' : 'Project build demo'}
      tabIndex={-1}
      className="fixed inset-0 z-modal flex items-center justify-center p-3 sm:p-5 bg-black/60 backdrop-blur-sm outline-none"
    ...
    >
      <div className="relative w-full max-w-5xl rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl flex flex-col border border-slate-200"
        ...
        <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-slate-100 shrink-0">
          ...
          <div className="flex items-center gap-3">
            {running && (
              <div className="flex items-center gap-2 bg-blue-50 text-blue-600 text-xs font-semibold px-3 py-1 rounded-full">
                <div className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse" />
                {lang === 'ru' ? 'Создаю' : 'Building'} {timeLeft}s
              </div>
            )}
            {finished && (
              <div className="flex items-center gap-2 bg-success text-success text-xs font-semibold px-3 py-1 rounded-full">
                <Icon name="CheckCircle" size={13} />
                {lang === 'ru' ? 'Рабочий проект + код в GitHub' : 'Working site + code in GitHub'}
              </div>
            )}
        ...
        {finished && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 sm:px-5 py-3 border-t border-slate-100 bg-white shrink-0 animate-fade-in">
            <div className="text-xs sm:text-sm text-slate-500">
              {lang === 'ru' ? 'Рабочий проект с бэкендом и код на Next.js + Prisma. Попробуйте сами.' : 'A working site with a backend and Next.js + Prisma code. Try it yourself.'}
            </div>
            <div className="flex gap-2 w-full sm:w-auto">
              <button onClick={reset} className="flex items-center gap-2 text-xs text-slate-400 hover:text-slate-600 transition-colors px-3 py-2 rounded-xl hover:bg-slate-100 shrink-0">
                <Icon name="RotateCcw" size={13} /> {lang === 'ru' ? 'Ещё раз' : 'Replay'}
              </button>
              <a href={getSession() ? '/builder' : '/register'} className="flex flex-1 sm:flex-none items-center justify-center gap-2 text-xs font-semibold bg-primary text-white rounded-full px-4 sm:px-5 py-2 hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
                {lang === 'ru' ? 'Создать свой проект' : 'Create my site'}
