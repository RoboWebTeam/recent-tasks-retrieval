...
export const getFEATURES = (lang: Lang) => lang === 'ru' ? [
  ...
  { icon: "Sparkles", title: "Code you can take", text: "The finished Next.js + Prisma project exports to your GitHub or GitFlic (the Russian analog). Real code, not a platform you rent forever." },
  ...
  { icon: "Search", title: "SEO out of the box", text: "Clean markup, meta tags and Next.js speed — a solid technical base for indexing in Google and Yandex." },
...
export const getCOMPARE = (lang: Lang) => lang === 'ru' ? [
  ...
  { label: 'Код проекта на вынос (в GitHub / GitFlic)', roboweb: 'Да', agency: 'За доплату', freelancer: 'Редко' },
  ...
  { label: 'Own the code (to GitHub / GitFlic)', roboweb: 'Yes', agency: 'Extra cost', freelancer: 'Rarely' },
...
export const getTRUST = (lang: Lang) => lang === 'ru' ? [
  { icon: "Github", value: "GitHub / GitFlic", label: "Весь код проекта — в вашем GitHub или GitFlic (рос. аналог), а не в нашей платформе" },
  ...
  { icon: "Github", value: "GitHub / GitFlic", label: "The whole project's code — in your GitHub or GitFlic (RU analog), not our platform" },
...
export const getPLANS = (lang: Lang) => lang === 'ru' ? [
  ...
  { name: 'Премиум', badge: 'популярный', price: '990 ₽', note: 'в месяц', tag: '30 генераций ежемесячно', features: ['Свой домен, без бейджа','Фуллстек-бэкенд: заявки, каталог, корзина, кабинеты','Экспорт кода Next.js + Prisma в GitHub или GitFlic','Публикация в GitHub Pages','Серверные функции и оформление заказа','Приоритетная сборка'], cta: 'Выбрать Премиум', hot: true },
] : [
  ...
  { name: 'Premium', badge: 'popular', price: '990 ₽', note: 'per month', tag: '30 generations/month', features: ['Custom domain, no badge','Fullstack backend: leads, catalog, cart, accounts','Export Next.js + Prisma code to GitHub or GitFlic','Publish to GitHub Pages','Server functions & checkout','Priority build'], cta: 'Choose Premium', hot: true },
];
...
export const getFAQ = (lang: Lang) => lang === 'ru' ? [
  ...
  { q: "Can I take the project's code with me?", a: "Yes, that's the key difference. The finished project exports as real Next.js + Prisma code to your GitHub or its Russian analog GitFlic (gitflic.ru) — frontend, backend, database schema and accounts. You're not locked into the platform: extend and host it anywhere." },
  ...
  { q: "You're a new product — why can I trust you with my project?", a: "Because the trust here isn't just words. The project's Next.js + Prisma code and all data in your PostgreSQL stay with you — even if we're gone. The stack is industry-standard, maintainable by any developer. There's a public offer and a registered company with a state ID (ОГРНИП/ИНН), verifiable on the state register. We show no invented clients or ratings — only what you can verify." },
  { q: "Can I get a refund?", a: "Yes. If the paid features weren't used, we refund within 7 days under the offer. Exact terms and timing are in the public offer." },
];
